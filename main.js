...
function computePublicKeyDigest(certBuffer) {
...
function fetchFromServer() {
  const pinnedDigest = computePublicKeyDigest(fs.readFileSync(CERT_PATH));
    ...
    const req = https.get(
      SERVER_URL,
      {
        ...
        checkServerIdentity: (hostname, cert) => {
          try {
            const digest = computePublicKeyDigest(cert.raw);
